...
export default function NavGroup({ item, level, matchDownLg, navigate, location, matchPath }) {
  ...
  const menus = item.children?.map((child) => {
    if (child.type === 'item') {
      return <NavItem key={child.id} item={child} level={level + 1} matchDownLg={matchDownLg} navigate={navigate} location={location} matchPath={matchPath} />;
    }
    return null;
  });
  ...
  return (
    <>
      ...
      >
        {Icon && (
          <ListItemIcon sx={{ color: textColor, minWidth: 28 }}>
            <Icon style={{ fontSize: '1.15rem' }} />
          </ListItemIcon>
        )}
        {!Icon && <DotIcon style={{ marginLeft: level === 1 ? 28 : 12, color: textColor, width: 4, height: 4 }} />}
        <ListItemText
