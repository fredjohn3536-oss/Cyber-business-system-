...
function Transitions({ children, position = 'top-left', type = 'grow', direction = 'up', ...others }, ref) {
  ...
  switch (position) {
    ...
    default: positionSX = { transformOrigin: '0 0 0' }; break;
  }

  return (
    <Box ref={ref}>
      {type === 'grow' && <Grow {...others} timeout={{ appear: 0, enter: 150, exit: 150 }}><Box sx={positionSX}>{children}</Box></Grow>}
      {type === 'collapse' && <Collapse {...others} sx={positionSX}>{children}</Collapse>}
      {type === 'fade' && <Fade {...others} timeout={{ appear: 0, enter: 300, exit: 150 }}><Box sx={positionSX}>{children}</Box></Fade>}
      {type === 'slide' && <Slide {...others} timeout={{ appear: 0, enter: 150, exit: 150 }} direction={direction}><Box sx={positionSX}>{children}</Box></Slide>}
...
export const PopupTransition = forwardRef(function PopupTransition(props, ref) {
